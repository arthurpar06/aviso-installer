use crate::aviso::InstallationStatus;
use std::collections::{HashMap, HashSet};
use std::fs;

/// Installs the AVISO content into the LFXX file.
///
/// # Arguments
/// * `lfxx_path` - Path to the local LFXX.sct file.
/// * `aviso_content` - Content of the AVISO file to install.
pub fn install_content(lfxx_path: &str, aviso_content: &str) -> Result<(), String> {
...
/// Checks if the Aviso content is already installed in the LFXX file.
pub fn is_installed(lfxx_path: &str, aviso_content: &str) -> Result<InstallationStatus, String> {
    let lfxx_bytes = fs::read(lfxx_path).map_err(|e| format!("Failed to read LFXX.sct: {}", e))?;
    let lfxx_content = String::from_utf8_lossy(&lfxx_bytes);

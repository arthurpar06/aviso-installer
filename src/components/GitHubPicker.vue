<template>
  <div class="h-full flex flex-col gap-6">
    <div class="flex gap-2 items-center justify-between shrink-0 px-1">
      ...
      >
        ...
        <a
          ...
          target="_blank"
          class="underline"
          >https://github.com/arthurpar06/lfxx-aviso</a
        >
      </div>
      <Button
        :disabled="loading"
    ...
    <div v-if="error" class="text-red-500 text-sm shrink-0 px-1">
      {{ error }}
    </div>

    <div
      v-if="displayedFiles.length > 0"
      class="flex-1 overflow-y-auto min-h-[400px] pr-2 custom-scrollbar"
    >
      <div
        class="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 gap-4 pb-4"
      >
        <div
        ...
        >
          ...
          <h3 class="font-medium text-base mb-4 text-center text-foreground/90">
            {{ file.name }}
          </h3>
          ...
          <Button
            ...
            @click="handleInstallClick(file)"
          >
            <Loader2
              v-if="installing === file.path"
    ...
    <div
      ...
      class="text-center text-muted-foreground text-sm py-12 shrink-0"
    >
      ...
      <DialogContent>
        ...
        <DialogFooter>
          <Button
            ...
            >Cancel</Button
          >
          <Button v-if="pendingFile" @click="confirmInstall"
